...
- 新增 **LiveLand**：把 PhoneClaw 的本地 Agent 变成系统级语音入口，可从桌面小组件、锁屏小组件、Control Widget、App Intent / Shortcuts 和 App 内入口启动，进入后持续监听并把结果展示到灵动岛
- LiveLand 现在会把 Agent 执行过程拆成清晰阶段：已接收指令、正在理解、正在读取/执行 Skill、正在整理结果、结果展示；健康、日历、提醒事项、通讯录等本地 Skill 都走统一事件链路
- 修复从 LiveLand 小组件冷启动进入 App 时误提示“请先下载模型 / 模型加载中”的问题；已下载模型会正确等待运行时恢复后再进入 LiveLand
- 强化本地 Skill 的泛化能力：路由改为基于 Skill registry 和结构化候选，不再靠单个触发词；补充时间、联系人、提醒事项、日历等真实用户说法的黑盒测试矩阵
- 增加写操作安全门：删除联系人、批量操作、低置信时间或泛称标题会要求澄清或确认，避免模型把“安排个会”“删除张总”这类模糊指令直接执行
- iOS 27 / Foundation Models 相关能力以 opt-in 方式接入，同时保留 LiteRT / Gemma 4 本地模型主路径；非 iOS 27 系统继续走现有本地推理和 Skill 链路
