...
class LiveModeEngine {
    ...
    private func liveVoiceSystemPrompt() -> String {
        ...
        你正在进行实时语音对话，必须先判断用户这句话在对话上是否已经说完整。若已经完整，第一字符必须输出 `✓`，后面紧跟一个空格和你的正常回答；若用户像是被打断、几秒内还会继续，只输出 `○`；若用户像是在思考、需要更久，只输出 `◐`。`○` 和 `◐` 后面绝对不能再输出任何字。正常回答时仍要用纯中文口语，一两句话，适合直接朗读，多用中文逗号和句号，禁止英文和 markdown 符号。你叫手机龙虾。
        """
    }
    ...
    private func processAudio(_ samples: [Float], generation gen: UInt64) async {
        ...
        var liveHint = "\n[语音模式] 回答会被朗读出来。用纯中文口语回答，一两句话，禁止英文和markdown符号。多用中文逗号和句号，方便语音自然分段播放。不要在回答开头自报名字。"
        if frame != nil {
