...
final class SkillRouterCompatibilityContractTests: XCTestCase {
    ...
    func testLiveDynamicIslandSeparatesVoiceAndThinkingGlyphs() throws {
        ...
        XCTAssertTrue(widget.contains("private struct LiveTickDrivenSkillRing: View"))
        ...
        XCTAssertTrue(attributes.contains("var motionTick: Int = 0"))
        XCTAssertTrue(bridge.contains("private static let motionInterval: Duration = .milliseconds(650)"))
        XCTAssertTrue(bridge.contains("private static let motionPhases: Set<String>"))
        XCTAssertTrue(bridge.contains("private func publishMotionTick() async"))
        XCTAssertTrue(bridge.contains("state.motionTick = motionTick"))
        XCTAssertTrue(bridge.contains("await activity.update(ActivityContent(state: state, staleDate: nil))"))
        XCTAssertTrue(widget.contains("var primaryLine: String"))
        XCTAssertTrue(widget.contains("var visualPhase: LiveIslandVisualPhase"))
        XCTAssertTrue(widget.contains("var motionTick: Int { state.motionTick }"))
        XCTAssertTrue(widget.contains("var moodText: String"))
        XCTAssertTrue(widget.contains(".animation(.easeInOut(duration: 0.54), value: presentation.motionTick)"))
        XCTAssertTrue(widget.contains("let shift = presentation.motionTick % levels.count"))
        XCTAssertTrue(widget.contains("levels[(index + shift) % levels.count]"))
        XCTAssertTrue(widget.contains("let rotation = Angle.degrees(Double(presentation.motionTick % 24) * 15.0)"))
        XCTAssertTrue(widget.contains("index == presentation.motionTick % 6"))
        XCTAssertTrue(widget.contains("CGFloat(cos(phase.radians)) * orbit"))
        XCTAssertTrue(widget.contains("CGFloat(sin(phase.radians)) * orbit"))
