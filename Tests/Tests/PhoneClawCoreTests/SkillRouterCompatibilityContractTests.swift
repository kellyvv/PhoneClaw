...
final class SkillRouterCompatibilityContractTests: XCTestCase {
    ...
    func testLiveDynamicIslandSeparatesVoiceAndThinkingGlyphs() throws {
        ...
        XCTAssertFalse(bridge.contains("Task.sleep"))
        ...
        XCTAssertTrue(widget.contains("TimelineView(.animation(minimumInterval: 1.0 / 30.0))"))
        XCTAssertTrue(widget.contains("frame(seconds: context.date.timeIntervalSinceReferenceDate)"))
        XCTAssertTrue(widget.contains("let wave = 0.5 + 0.5 * sin(seconds * 6.4 - Double(index) * 0.72)"))
        XCTAssertTrue(widget.contains("let rotation = seconds * 1.9"))
        XCTAssertTrue(widget.contains("let phase = Double(index) * .pi / 3.0 + rotation"))
        XCTAssertTrue(widget.contains("let pulse = 0.5 + 0.5 * sin(seconds * 7.2 - Double(index) * 0.86)"))
        XCTAssertTrue(widget.contains("CGFloat(cos(phase)) * orbit"))
        XCTAssertTrue(widget.contains("CGFloat(sin(phase)) * orbit"))
        ...
        XCTAssertTrue(widget.contains("ForEach(0..<6, id: \\.self)"))
