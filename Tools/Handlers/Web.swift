...
enum WebTools {
    ...
    static func register(into registry: ToolRegistry) {
        registry.register(RegisteredTool(
            ...
            parameters: tr(
                "query: 搜索关键词, question: 用户原问题（可选，用于保留时间/地点/约束）, max_results: 返回结果数（可选，默认 5，最多 8）。保留用户原始时间、地点和主体；不要把“今天/最新”机械改写成年份。",
                "query: search query, question: original user question (optional, preserves time/location/constraints), max_results: number of results (optional, default 5, max 8). Preserve the user's original time, location, and subject; do not mechanically rewrite “today/latest” into a year."
            ),
    ...
    private static func searchCanonical(_ args: [String: Any]) async -> CanonicalToolResult {
        ...
        let plan = makeSearchPlan(originalQuery: originalQuestion, normalizedQuery: providerQuery, fetchedAt: fetchedAt)
    ...
    private static func makeSearchPlan(
        ...
        var candidates: [String] = [normalizedQuery, originalQuery]
